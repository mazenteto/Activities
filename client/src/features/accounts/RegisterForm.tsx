...
export default function RegisterForm() {
    ...
    const{control,handleSubmit,setError,formState:{isValid,isSubmitting}}=useForm<RegiserSchema>({
        mode:'onTouched',
        resolver:zodResolver(regiserSchema)
    });
    const onSubmit=async (data:RegiserSchema)=>{
        await registerUser.mutateAsync(data,{
            onError:(error)=>{
                ...
                else if(err.includes('DisplayName')) setError('displayName',{message:err});
                })
            }
        });
    }
